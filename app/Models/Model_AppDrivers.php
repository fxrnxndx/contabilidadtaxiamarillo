...
class Model_AppDrivers extends Model
{
    ...
    function login($user,$pass){
        ...
        chofer.apellidos as apellidos
        FROM tbl_usuarios as user
        INNER JOIN tbl_chofer as chofer ON user.id_perfil = chofer.numero_empleado 
        WHERE user.usuario='{$user}' AND user.contra='{$pass}' AND user.estatus='ACTIVADO' and user.id_tipo_usuario_fk=3";
...
function InsertarVenta($destino,$descripcion,$id_vendedor,$id_empresa,$id_chofer,$total,$totalMXN,$fecha_venta,$id_moneda,$id_tipo_pago,$id_tipo_tarjeta)
{
  $sql = "INSERT INTO tbl_ventas (destino,descripcion,id_vendedor_fk,id_chofer_fk,id_empresa_fk,total,totalMXN,id_tipo_pago_fk,id_tipo_tarjeta_fk,id_moneda_fk,fecha_venta) 
  VALUES ('{$destino}','{$descripcion}','{$id_vendedor}','{$id_chofer}','{$id_empresa}','{$total}','{$totalMXN}','{$id_tipo_pago}','{$id_tipo_tarjeta}','{$id_moneda}','{$fecha_venta}')";
  $query= $this->db->query($sql);
...
function BuscarVentaPorChofer($fecha_venta,$idchofer)
{
  $sql = "SELECT venta.*,empresa.nombre as nomemp
  FROM tbl_ventas as venta
  INNER JOIN tbl_empresa as empresa ON venta.id_empresa_fk = empresa.id_empresa 
  WHERE venta.fecha_venta LIKE '%{$fecha_venta}%' and venta.id_chofer_fk='{$idchofer}' and venta.estatus='VENDIDO'";
  $query= $this->db->query($sql);
