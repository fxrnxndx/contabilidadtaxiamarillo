...
class AppControll extends BaseController
{
	...
	public function login()
	{
		...
		foreach ($TodosChofer['TodosChofer'] ->getResult() as $row) { 
			$resultado=$row->numero_empleado.'- '.$row->nombre.' '.$row->apellidos.'- '.$row->numunidad.'- '.$row->unidadplacas.'- '.$row->unidadmarca.' '.$row->unidadmodelo.','.$resultado;
		   /* if($resultado=="")
			{
		...
		foreach ($TipoCambio['TipoCambio'] ->getResult() as $row) { 
			$tipodecambio=$row->valor;
		   /* if($resultado=="")
			{
				$resultado=$row->nombre.' '.$row->apellidos.'-'.$row->empresanom;
			}
		...
		}else
		{
			...
			if($tipoperfil=="2")
			{
				return $idperfil."*".$resultado."*".$tipodecambio;
			}
			else{
				return "9";
			}	  	
...
*/
	...
	public function venta2()
	{
		...
		if($num_unidad==""){
			$num_unidad=explode("-", "0- ");
			
		}else
		{ 	$num_unidad=explode("-", $num_unidad);
		...
		if($idmaxventa!=false)
			{
				...
				if($Ventarepetida['Ventarepetida']!=false)
				{
					...
					{ // cuando el ticekt no se repite
						...
						//insertamos nueva venta
						$this->model->InsertarVenta($destino,$descripcion,$id_usuario,$id_empresa,$id_chofer[0],$total,$totalMXN,$fecha_venta,$id_moneda,$id_tipo_pago,$id_tipo_tarjeta,$pasajero,$punto_venta,$num_unidad[0]);
						//Extraemos el id de la venta
						$DatosVenta = array(
							'DatosVenta' => $this-> model-> BuscarVenta($fecha_venta,$id_empresa)
							);
						...
						if($id_empresa==1){
							   $ticket=$id_venta;
							  $correo="facturacion@taxisairport.com";
							  $nom_empresa="TT";
						}
						else if($id_empresa==2)
						{
			           ...
			           // print ('<img src="https://contabilidad.taxisairport.com/assets/images/qr.png" alt="Mi Foto" width="50">');
		...
		public function Reservar()
	{
		...
		$dataCliente = $this->model->BuscarCliente($telefono);
	...
	public function ObtenerUnidad(){
		...
		foreach ($TodasUnidades['TodasUnidades'] ->getResult() as $row) { 
			$resultado=$row->NumUnidad.'- '.$row->Placas.' -'.$row->Marca.' '.$row->Modelo.','.$resultado;
